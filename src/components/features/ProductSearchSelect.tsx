    ...
    <div ref={boxRef} className={cn('relative', className)}>
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-white border border-slate-200 rounded-xl py-2 pr-8 pl-8 text-sm text-slate-800 focus:outline-none focus:border-gold-400"
        />
        {selected && !open ? (
          <button
            type="button"
            onClick={() => { onChange('', null); setQuery(''); }}
            className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-300 hover:text-red-400"
          >
            <X className="w-3.5 h-3.5" />
          </button>
        ) : (
          <ChevronDown className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-300 pointer-events-none" />
        )}
      </div>

      {open && (
        <div className="absolute top-full right-0 left-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl z-30 max-h-56 overflow-y-auto">
          {filtered.length === 0 ? (
            <div className="px-3 py-3 text-xs text-slate-400 text-center">مفيش نتائج</div>
          ) : (
            filtered.map(p => (
              <button
                key={p.id}
