          ...
          return (
            <div key={i}
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="text-xs font-semibold text-slate-400 mt-0.5">{card.label}</span>
              </div>
              <p className="text-xl font-black leading-tight break-all" style={{ color: card.valueColor }}>{card.value}</p>
              <p className="text-xs text-slate-400 mt-1.5 leading-tight">{card.sub}</p>
            </div>
          );
        })}
      </div>
            ...
            return (
              <button key={i} onClick={() => navigate(sc.path)}
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{sc.sub}</p>
                </div>
              </button>
            );
          })}
